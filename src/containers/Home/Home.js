...
export default class Home extends Component {
  ...
  render() {
    ...
    getBlogEntriesState.articles.forEach(function(entry){
      ...
      blogContentDef.push(
        <div onClick={() => this.loadArticle(entry.articleId)} className={styles.topLine + ' animated fadeInUp col-xs-12 ' + styles.hover}>
          <div className='row'>
            ...
            <div className={'col-sm-3 col-xs-12 ' + styles.techStyle}>{ Object.keys(entry.technologies[0]).map(entry.technologies[0][key].length > 1 ? (key => <span title={entry.technologies[0][key]} className={stylesCommunity.cbs00Home + ' ' + stylesCommunity['cbs'+key]}></span>) : '' ) }</div>
