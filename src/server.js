        ...
        if (err) {
          ...
          CommentsOrRatingData.save(function (err) {
              if (err) {
                ...
                res.json({ status: 1 }); /* Created successfully first RATE */
              }
          });
        }
        else {
          res.json({ status: 1 }); /* updated successfully RATE */
        }
      });
    } else{ /* comments: comments are not limited to 1 */
      CommentsOrRatingData.save(function (err) {
          if (err) {
            return console.log(err);
            res.json({ status: 0 });
...
app.post('/getNewestUsers', function(req, res) {
      UserModel.find({ activation: true }).limit(30).sort({date: -1}).exec(function(err, result) {
        if(err){
          res.json(err);
          res.json({ status: 0 });
        }
        else{
          res.json({ status: 1, userArticles: result });
        }
      });
});
